...
public class SpawnerSingleObject : MonoBehaviour
{
    [SerializeField]
    ...
    public async void SpawnWithEvent()
    {
        ...
        _cashedObject.GetComponent<UnloadObject>().SetLoader(_loader);
    }

    public async void SpawnWithEventWithInject()
    {
        EnableEvent.CallEnabled(true);

        var handle = _loader.LoadWithInject(_assetReference, _positionRoot);
